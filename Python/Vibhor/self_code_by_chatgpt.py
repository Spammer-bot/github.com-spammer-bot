...
class Vibhor:
    ...
    def add_friends(self):
        ...
        add_butn = tk.Button(self.root, text="Add Friends", command=self.add_friends2, bg="#4e4e4e", fg="white", font=("Arial", 20))
    ...
    def add_friends2(self):
        task = self.friends_entry.get().strip()
        if not task:
        ...
        check = tk.Checkbutton(
            self.scrollable_frame, text=task, variable=var, onvalue=1, offvalue=0, bg="#2e2e2e", fg="white", selectcolor="#9e9e9e", font=("Arial", 12), anchor="w", padx=10, pady=5 
        )
        check.pack(fill=tk.X, anchor="w")

        delete_friend_butn = tk.Button(
            self.scrollable_frame,
            text="Delete Friend",
            command=lambda c=check, b=None: self.delete_friend(c, delete_friend_butn),
            bg="red",
            fg="white",
            font=("Arial", 10)
        )
    ...
    def delete_friend(self, checkbox, button=None):
        checkbox.destroy()
        if button:
            button.destroy()
